...
- (void)captureImageWithCompletionHander:(void(^)(id data, CGFloat confidence))completionHandler
{
    if (_isCapturing) {
        return;
    }
    
    if (_imageDedectionConfidence < 10 || !_foundRectangle) {
        return;
    }
    
    NSLog(@"Confidence and rectangle good enough! %@", @(_imageDedectionConfidence));
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error)
     {
             ...
             if (weakSelf.isBorderDetectionEnabled) {
                ...
                completionHandler(image, _imageDedectionConfidence);
             }
